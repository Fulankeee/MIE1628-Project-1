...
public class KMeanKC extends Configured implements Tool{
...
//    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length < 2) {
            System.err.println("Usage: KMeanK2 <input_path> <output_path>");
            System.exit(1);
        }

        Configuration conf = new Configuration();
        Path canopyOutputPath = new Path("canopy_output");
        conf.set("centroid.path", canopyOutputPath.toString());

        // Step 1: Run Canopy Clustering to generate initial centroids
        System.out.println("Running Canopy Clustering...");
        CanopyClustering.runCanopyClustering(args[0], canopyOutputPath.toString(), conf);

        // Step 2: Load initial centroids from Canopy Clustering output
        System.out.println("Loading initial centroids from Canopy Clustering...");
        List<Point> initialCentroids = CanopyClustering.loadCanopyCenters(canopyOutputPath.toString(), conf);
        Point.writePointsToFile(initialCentroids, conf);

        long startTime = System.currentTimeMillis();

        // Step 3: Run K-Means Iteratively
        int iteration = 1;
        ...
        while (iteration <= maxIterations && !converged) {
            ...
            // Ensure output directory does not exist (required by Hadoop)
            if (fs.exists(outputPath)) {
                fs.delete(outputPath, true);
            }

            Job job = Job.getInstance(conf, "KMeans Iteration " + iteration);
            job.setJarByClass(KMeanKC.class);
